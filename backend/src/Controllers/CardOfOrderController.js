...
class CardOfOrderController {
    ...
    async deleteAll(req, res) {
        ...
        try {
            ...
            console.error(err.message);
            return res.status(400).send("Error: Failed to delete records! " + err.message);
        }
    }
    async deleteOne(req, res) {
        const id = req.params.id;
        try {
            const result = await pool.query(`SELECT id FROM cardoforder WHERE id = $1`, [id]);
            if (result.rows.length === 0) {
            ...
            console.error(err.message);
            return res.status(400).send("Error: Failed to delete record! " + err.message);
        }
    }    
