...
class CardOfOrderController {
    ...
    async getAll(req, res) {
        ...
        pool.query(sql, [], (err, result) => {
            ...
            const formattedData = result.rows.map(row => ({
                ...row,
                created: new Date(row.created).toLocaleString('en-US', { timeZone: 'Europe/Moscow' })
            }));
    ...
    async getOne(req, res) {
        ...
        pool.query(sql, [id], (err, result) => {
            ...
            const formattedData = {
                ...result.rows[0],
                created: new Date(result.rows[0].created).toLocaleString('en-US', { timeZone: 'Europe/Moscow' })
            };
            res.json(formattedData); 
        });
    }
    ...
    async deleteAll(req, res) {
        const sql_count = "SELECT COUNT(*) FROM cardoforder"; // Подсчитать количество записей в таблице client
        pool.query(sql_count, (err, result) => {
            ...
            const sql_delete = "DELETE FROM cardoforder"; // Удалить все записи из таблицы client
    ...
    async deleteOne(req, res) {
        ...
        pool.query(sql_exist, [id], (err, result) => {
            ...
            const sql_delete = `DELETE FROM cardoforder WHERE id = $1`;
    ...
    pool.query(sql_exist, [id], (err, result) => {
        ...
        pool.query(sql_update, [id, price, description, created, ended, client_id], (err, result) => {
            if (err) {
                console.error(err.message);
                return res.status(400).send("Error: Failed to update card record! " + err.message);
            }
